...
# Class to manage the OpenAI-API, which is used to ask ChatGPT
class OpenAiManager:

    # Initialize class and set history as empty, also set API-Key
    def __init__(self):
        self.chat_history = [] # Stores the entire conversation
        try:
            self.client = OpenAI(api_key = os.environ['OPENAI_API_KEY'])
        except TypeError:
            exit("The OPENAI_API_KEY environment variable is not set!")

    # Send prompt to AI and return answer
    ...
    def chat_with_history(self, prompt = ""):
        ...
        # Add our prompt into chat history
        ...
        while num_tokens_from_messages(self.chat_history) > 8000:
            self.chat_history.pop(1) # We skip 1st message since it's the system message
...
if __name__ == '__main__':
    ...
    while True:
        new_prompt = input("\nType out your next question to Jack Sparrow, then hit enter: \n\n")
