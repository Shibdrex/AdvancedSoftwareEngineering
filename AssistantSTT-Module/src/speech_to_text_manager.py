...
# Class to manage the azure STT API, which i used to get text from the users audio input
class SpeechToTextManager:
    azure_speechconfig = None
    azure_audioconfig = None
    azure_speechrecognizer = None

    # Initialize class and set API-Key and Region for azure
    def __init__(self):
        try:
            self.azure_speechconfig = speechsdk.SpeechConfig(subscription = os.getenv('AZURE_TTS_KEY'), region = os.getenv('AZURE_TTS_REGION'))
        except TypeError:
            exit("AZURE_TTS_KEY or AZURE_TTS_REGION is not set as environment variable!")
        
        # Set language to be recognized
        self.azure_speechconfig.speech_recognition_language="de-DE"
    
    # Records speech from microphone input, stops when no speec is detected or after 30 seconds
    ...
    # Records speech from a audio file
    ...
    # Records speech from audio file, file can get longer while recording
    ...
    # Records speech from microphone input, stops recording when stop key is pressed
